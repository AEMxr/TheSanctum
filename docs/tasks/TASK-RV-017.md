# TASK-RV-017 - Emit deterministic audit record output

## Objective lane
revenue

## Goal (single measurable outcome)
On successful `lead_enrich`, emit a deterministic `audit_record` object derived from `telemetry_event` that is directly consumable by downstream compliance/evidence exporters.
...
- [ ] Successful `lead_enrich` emits `result.audit_record`.
- [ ] `audit_record` includes:
  - `record_id`
  - `event_id`
  - `receipt_id`
  - `request_id`
...
- [ ] `accepted_action_types` is deterministic and ordered exactly:
  1) `cta_buy`
  2) `cta_subscribe`
- [ ] Action mapping is deterministic from `telemetry_event.accepted_action_types`:
  - includes only action-type lineage in stable order
- [ ] `record_id` is deterministic from stable lineage inputs (task/campaign/channel/variant/event), with no timestamp/randomness.
- [ ] `audit_record.reason_codes` includes:
...
- [ ] `audit_record` is deterministic across repeated runs for identical input.
- [ ] `audit_record` contains no precise geo or direct-contact fields (`latitude`, `longitude`, `email`, `phone`, `ip_address` absent).
- [ ] FAILED/malformed `lead_enrich` path does **not** emit `audit_record`.
...
- Diff summary:
  - Revenue route result adds deterministic `audit_record` for successful `lead_enrich`.
  - Top-level revenue index surfaces `audit_record`.
  - Smoke tests validate record shape, ordering, lineage, privacy, and determinism.

## Rollback anchor
`rollback/TASK-RV-017-pre`

## Commit message (exact)
`feat(revenue): emit deterministic audit record output (TASK-RV-017)`
...
- `record_id` must be deterministic and stable across retries for identical input.
- Next first command: `Get-Content docs/tasks/TASK-RV-017.md -Raw`
